...
/// <summary>
/// Acerca la cámara al BreakerBoxManager. Pulsa E mirando el BreakerBoxManager (o muy cerca) · Escape = salir.
/// </summary>
public class BreakerBoxCameraFocus : MonoBehaviour
{
    [SerializeField] private Camera playerCamera;
    [SerializeField] private FirstPersonController firstPersonController;
    [SerializeField] private CharacterController characterController;

    [Header("Vista del BreakerBoxManager")]
    ...
    [SerializeField] private BreakerBoxManager breakerBoxManager;
    ...
    [Tooltip("Si está activado, al acertar el PIN se destruye el objeto del punto de vista del BreakerBoxManager (para que no vuelva a salir el prompt).")]
    ...
    private bool _BreakerBoxManagerUnlocked;
    ...
    {
        ResolvePlayerCamera();
        ResolveBreakerBoxManager();
        SetPromptVisible(false);

        if (breakerBoxManager != null)
        {
            breakerBoxManager.OnAccessGranted.AddListener(OnBreakerBoxManagerAccessGranted);
        }
    }

    public bool IsBreakerBoxManagerUnlocked => _BreakerBoxManagerUnlocked;

    private void OnDestroy()
    {
        if (breakerBoxManager != null)
        {
            breakerBoxManager.OnAccessGranted.RemoveListener(OnBreakerBoxManagerAccessGranted);
        }
    }

    private void ResolveBreakerBoxManager()
    {
        if (breakerBoxManager != null)
        {
            return;
        ...
        breakerBoxManager = FindAnyObjectByType<BreakerBoxManager>();
    }

    private void OnBreakerBoxManagerAccessGranted()
    {
        if (_BreakerBoxManagerUnlocked)
        {
            return;
        }

        _BreakerBoxManagerUnlocked = true;
        SetPromptVisible(false);
        // _selectManager?.SetBreakerBoxManagerInteractionEnabled(false);
    ...
    private IEnumerator HandleUnlockRoutine()
    {
        ...
        CleanupAfterUnlock();
        DisableLegacyBreakerBoxManagerPrompts();
        _unlockRoutine = null;
    }
    ...
    private static void DisableLegacyBreakerBoxManagerPrompts()
    {
    ...
    private void ForceRestoreCameraImmediate()
    {
        ...
        RestoreFpsCursorAfterBreakerBoxManager();
    }
    ...
    private void Update()
    {
        ...
        var canInteract = CanInteractWithBreakerBoxManager();
    ...
    private bool CanInteractWithBreakerBoxManager()
    {
        if (focusViewpoint == null || playerCamera == null)
        {
            return false;
        ...
        if (RaycastHitsBreakerBoxManager(playerCamera.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0f))))
        {
            return true;
        }
    ...
    private bool RaycastHitsBreakerBoxManager(Ray ray)
    {
        ...
        return hit.collider.GetComponentInParent<BreakerBoxManager>() != null
               || hit.collider.GetComponentInParent<BreakerSwitch>() != null;
    }
    ...
    private void SetPromptVisible(bool visible)
    {
        ...
        if (visible)
        {
            PlayerInteraction.GlobalHoverText = "[ Presiona 'E' para ver Caja ]";
        }
    }
    ...
    private void RestoreFpsCursorAfterBreakerBoxManager()
    {
